...
- 양쪽 모두에 값을 세팅
  ...
  - 무한루프에 빠질 수 있다.(toString(),lombok,JSON 등)
...
- 알(1)이 주인
- 다쪽에 외래 키가 있음
- 일 쪽에서 @joinColumn 이용하여 매핑
- 쿼리가 외래 키를 가진 테이블때문에 더 실행해야 된다.
- @joinColumn 사용하지 않으면 joinTable 방식을 사용 ( 중간 테이블을 하나 생성함 )
...
- 기본적으로 조인전략을 가져가되 단일과 조인의 장단점을 가지고 고민해봐야 한다.
- 단순한 테이블에는 단일 테이블이 효과적
- 비지니스적으로 복잡하거나 확장성이 있다면 조인전략이 효과적
#### 조인전략
- @DiscriminatorColumn 으로 테이블을 특정할 수 있음
- @DiscriminatorValue 으로 각 테이블별로 설정 가능 (ex. movie -> M)
장점
- 정규화가 되어있음
...
- insert 2번 호출
#### 단일 테이블 전략
- 어노테이션이 없어도 DTYPE 은 기본으로 세팅됨
장점
- 조인 필요없음
- 쿼리가 단순함
단점
- 자식 엔티티가 매핑한 컬럼은 모두 null 허용
- 테이블이 커지거나 데이터가 많아지면 상황에 따라서 성능 저하 가능
#### 구현 클래스마다 테이블 전략
- 사용하지 말아야할 전략
장점
- 서브 타입을 명확하게 구분할땐 효과적
- not null 제약조건 사용 가능
단점
- 여러 테이블을 함께 조회하면 성능 저하 ( union 사용 )
- 자식 테이블 통합이 어려움
### @MappedSuperclass
- 공통 매핑 정보가 필요할 때 사용 ( 생성자, 생성일자 같은 공통적인 컬럼 )
- 공통 컬럼항목의 class를 만들고 @MappedSuperclass 어노테이션을 붙인다.
- 해당 클래스를 엔티티들은 상속을 바는다. ( extend )
- 상속관게 매핑 X
- Entity X , 테이블 매핑 X
- 자식클래스에 매핑 정보만 제공
- 조회 불가 ( em.find(BaseEntity) 불가)
- 직접 생성해서 사용할 일이 없으므로 추상 클래스 권장
- jpa 는 @Entity 나 @MappedSuperclass 만 상속 가능